...
os.makedirs('%s/gen_1/' % opt.log_dir, exist_ok=True)
...
#        mmse = mse_criterion(x_pred, x[i])
    ...
    # print(mmse_1)
    # print('$$$$')
    # index_min=np.min(mmse_1)
    # index_max=np.max(mmse_1)
    
    # for i in range(len(mmse_1)):
    #     score=(mmse_1[i]-np.min(mmse_1))/(np.max(mmse_1)-np.min(mmse_1))
    #     score_1.append(score)
    #     #print((np.max(mmse_1)-np.min(mmse_1))
    #     print(score)
    #     print('####')

    #print("Index of anomaly: %.2d and the score: %.5f",np.min(mmse_1))
