...
def image_tensor(inputs, padding=1):
    ...
    if is_sequence(inputs[0]) or (hasattr(inputs, "dim") and inputs.dim() > 4):
        ...
        #     noise = np.random.normal(0,1, 64).T
        #     result[:, result[i]+noise : result[i]+noise, :]
...
def make_image(tensor):
    ...
    tensor=tensor.numpy().astype('float32')
    # pdb.set_trace()
    #print(tensor.numpy())
    return scipy.misc.toimage(tensor, high=255*tensor.max(), channel_axis=0)
...
def score(l_pred,gt):
    ...
    for a in range(bs):
        for b in range(T):
            lpred_min = np.min(l_pred)
            lpred_max = np.max(l_pred)
            zcore = ((l_pred[b] - lpred_min)/(lpred_max - lpred_min))
            score_l.append(zcore)
        #print("score", score_l)
...
#     N = len(test_data)
#     print(N)
#     #print('test sahpe',test_data.shape)
#     # plt.imshow(test_data[0,:,:,0],cmap='gray')
#     # plt.show()
